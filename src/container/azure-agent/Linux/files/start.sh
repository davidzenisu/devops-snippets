...
cleanup() {
  if [ -e config.sh ]; then
    # https://github.com/kedacore/keda/discussions/2649#discussioncomment-2432405
...
if [ -z "$AZP_PLACEHOLDER" ]; then 
  ...
  echo "Agent marked as a placeholder"
  print_header "5. Immediately shutting down agent"
  echo "This option should be removed if Azure DevOps allows queueing to empty Agent Pools."
  ./run.sh "$@" & exit 0
fi